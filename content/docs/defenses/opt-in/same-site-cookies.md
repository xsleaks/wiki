...
{{< hint info >}}
One way to work around this would be to redirect using a same-site html meta tag,
Like `<meta http-equiv="refresh" content="0;/" />`.
Then unlike when using the `Location` header the initiator will be changed to same-origin.
The redirect should be to a page safe from timing attacks or the redirector should set a `cross-origin-opener-policy` header.
{{< /hint >}}

...
- If a page contains images with [Lazy Loading](https://web.dev/native-lazy-loading/), an attacker might known whether a fragment navigation occurred by checking if an image was cached in the browser by [probing the Cache](https://TODO) (only if the image appears after the navigation). This occurs because [Lazy Loading](https://web.dev/native-lazy-loading/) images are only cached when they appear in the viewport.
